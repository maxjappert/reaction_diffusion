...
public class Main_cool2 extends PApplet {
    ...
    public void draw() {
        ...
        for (int i = 0; i < numCells; i++) {
            for (int j = 0; j < numCells; j++) {
                ...
                //println(A);
                //println(B);
                float A_prime = A + (D_A * convolution(i, j, 1)-A*B*B + f*(1 - A))*delta_t;
                float B_prime = B + (D_B * convolution(i, j, 0)+A*B*B - (k + f)*B)*delta_t;
